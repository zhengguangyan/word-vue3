<template>
  <el-dialog class="sever-dialog" destroy-on-close v-model="visibleFlag" title="选择关联服务" width="778px" @close="onClose" :close-on-press-escape="false" :close-on-click-modal="false">
    <div class="serve-main">
      <el-input v-model="queryForm.serveName" placeholder="搜索服务类别/服务项目" @change="getList" clearable style="width: 100%; margin-bottom: 5px">
        <template #suffix>
...
const getList = async () => {
  const { data } = await getServeWithSellerServePage({
