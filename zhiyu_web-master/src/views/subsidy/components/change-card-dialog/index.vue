<template>
  <el-dialog destroy-on-close v-model="visibleFlag" title="换卡" width="508px" @close="onClose" :close-on-press-escape="false" :close-on-click-modal="false">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="原卡号">
        <span>{{ form.code }}</span>
      </el-form-item>
      <el-form-item label="状态" prop="lossStatus">
        <DictSelect width="195px" v-model="form.lossStatus" :clearable="false" :distList="statusList" />
      ...
      <el-form-item label="新卡号" prop="newCardId">
        <el-space>
          <el-input v-model="form.newCode" disabled />
          <el-button link type="primary" @click="openAddCord"> 选择 </el-button>
        </el-space>
      </el-form-item>
      <el-form-item label="备注" prop="remark">
        <el-input v-model="form.remark" :rows="3" type="textarea" placeholder="请输入换卡原因" />
      </el-form-item>
    </el-form>
    <template #footer>
  ...
  <AddCardDialog ref="AddCardRef" @change-card="handleCard" :dict="dict" :isChange="true" />
</template>
<script lang="ts" setup>
import { ref, reactive, computed } from 'vue'
import { ElMessage, FormRules } from 'element-plus'
import { changeCard } from '@/api/subsidy'
import DictSelect from '/@/components/DictSelect/index.vue'
import AddCardDialog from '../add-card-dialog/index.vue'
...
const statusList = ref([{ value: '3', label: '换卡' }])
...
const rules = reactive<FormRules>({
  newCardId: [{ required: true, message: '请选择状态', trigger: 'change' }],
...
const handleCard = (row) => {
  form.newCode = row.code
  form.newCardId = row.id
}
