  ...
  <el-card shadow="never" class="card">
    <template #header>
      <div class="card-header">
        <span class="title">
          迁入迁出记录：{{ query.title }}{{ changeType }}{{ movingType }}
          <el-tooltip class="box-item" effect="dark" placement="right-start">
            ...
            <Icon icon="ant-design:exclamation-circle-outlined" style="vertical-align: middle; padding-bottom: 2px" />
          </el-tooltip>
        </span>
      </div>
    </template>
    <div class="query-form">
      <el-space wrap :size="12">
        ...
        <div>
          ...
          <el-date-picker v-model="queryForm.date" type="daterange" value-format="YYYY-MM-DD" format="YYYY-MM-DD" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" />
        </div>
    ...
    <el-table :data="tableData" style="height: calc(100vh - 310px)" @selection-change="handleSelectionChange" class-name="table-without-border" border>
      ...
      <!-- <el-table-column show-overflow-tooltip prop="type" label="操作类型" width="100">
        <template #default="{ row }">
          {{ changeType }}{{ row.movingMatch == 0 ? '迁入' : '迁出' }}
        </template>
      </el-table-column> -->
      ...
      <el-table-column show-overflow-tooltip prop="newAddress" :label="'新地址(' + changeType + ')'" min-width="300">
        <template #default="{ row }">
          {{ changeType == '现居' ? row.newCurrentRegionName : row.newDomicileRegionName }}
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip prop="oldAddress" :label="'原地址(' + changeType + ')'" min-width="300">
        <template #default="{ row }">
          {{ changeType == '现居' ? row.oldCurrentRegionName : row.oldDomicileRegionName }}
        </template>
      </el-table-column>
    </el-table>
...
import { getAddressChangeRecordPage, moveInOutExport } from '@/api/customer'
...
const exportSubmit = async (form: any) => {
  ...
  if (form.data === 'select') {
    ...
    const data = await moveInOutExport(exportForm)
    download.excel(data.data, '迁入迁出记录.xlsx')
...
async function getTableData() {
  ...
  const res = await getAddressChangeRecordPage({
    ...queryForm,
    areaCode: areaCode,
    movingMatch: Number(query.movingMatch),
...
const name = ref()
const movingType = ref()
const changeType = ref()
onMounted(() => {
  name.value = wsCache.get('areaName')
  movingType.value = Number(query.movingMatch) == 0 ? '迁入' : '迁出'
