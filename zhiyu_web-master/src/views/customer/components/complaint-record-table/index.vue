<template>
  ...
  >
    ...
    <el-table-column show-overflow-tooltip prop="worksheetCategory" label="投诉类型" sortable="custom" min-width="120">
      <template #default="{ row }">
        {{ filterWorksheetType(row.worksheetCategoryId) }}
      </template>
    </el-table-column>
    <el-table-column show-overflow-tooltip prop="worksheettypeInfo" label="投诉类别" sortable="custom" min-width="120">
      <template #default="{ row }">
    ...
    <el-table-column show-overflow-tooltip prop="returnUserName" label="回访人" sortable="custom" min-width="120" />
  </el-table>
...
function add() {
  push(`/service/add?serviceType=complaint&id=${props.customerId}&phone=${props.phone}`)
}
...
const queryFormDefault = {
  worksheetTypeId: 3965352075919877,
