...
export default defineComponent({
  ...
  setup(props, { slots, expose, emit }) {
    ...
    const renderFormItem = (item: FormSchema) => {
            ...
            <>
              ...
              <ElTooltip placement="right" raw-content>
                {{
                  ...
                  default: () => <Icon icon="ep:warning" size={16} color="var(--el-color-primary)" class="ml-2px relative top-1px"></Icon>
                }}
              </ElTooltip>
            </>
          )
      ...
      const { showFormLabel, labelPosition } = unref(getProps)
      return (
        // 这边需要去掉文本(搜索表单的时候) label={item.label || ''}
        <ElFormItem {...(item.formItemProps || {})} label-position={labelPosition} prop={item.field} label={showFormLabel ? item.label || '' : ''}>
          {{
            ...formItemSlots,
            default: () => {
    ...
    return () => (
      <ElForm ref={elFormRef} {...getFormBindValue()} model={props.isCustom ? props.model : formModel} class={prefixCls}>
        {{
