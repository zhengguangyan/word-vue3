    ...
    >
      <template #reference>
        <div :style="iconWrapStyle">
          ...
          <Icon v-else icon="ant-design:search-outlined" />
        </div>
      </template>
        ...
        <ElTabs tab-position="left" v-model="iconName" @tab-change="tabChange">
          <ElTabPane v-for="item in icons" :key="item.name" :label="item.name" :name="item.prefix">
            <div class="flex flex-wrap box-border">
              <div
                v-for="i in filterIcons(filterItemIcons(item.icons))"
                :key="i"
                :style="{
                  width: iconSize,
                  height: iconSize,
                  ...
                  cursor: 'pointer',
                  border: `1px solid ${
                    i === modelValue ? 'var(--el-color-primary)' : 'var(--el-border-color)'
                  }`,
              ...
              >
                <Icon
                  :icon="i"
                  :color="i === modelValue ? 'var(--el-color-primary)' : 'inherit'"
                />
              </div>
            </div>
          </ElTabPane>
