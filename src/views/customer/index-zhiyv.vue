<template>
    ...
    <el-card shadow="never" class="card">
      <template #header>
        <div class="card-header">
          <span class="title">{{ activeIndex == '4' ? '开放平台客户' : '智宇客户' }} </span>
      ...
      <div class="query-form">
        <el-space wrap :size="12">
          <template v-if="activeIndex == '4'">
            <el-button type="primary" @click="addCustomer" :disabled="!leafAreaFlag || getAuth('customer:open:add')"> 添加客户 </el-button>
            <!-- <el-button :disabled="!leafAreaFlag || getAuth('customer:open:allot')">分配</el-button> -->
            <!-- <el-button :disabled="!leafAreaFlag || getAuth('customer:open:packet')">生成数据包</el-button> -->
            <el-button :disabled="!leafAreaFlag || getAuth('customer:open:equipment')">批量关联设备卡</el-button>
            <el-button :disabled="getAuth('customer:open:analyse')" @click="showAnalysisOptionsDialog">数据分析</el-button>
            <el-button @click="showExportDialog" :disabled="getAuth('customer:open:export')">导出</el-button>
          </template>
          <template v-else>
            <el-button type="primary" @click="addCustomer" :disabled="!leafAreaFlag || getAuth('customer:zhiyv:add')"> 添加客户 </el-button>
            <!-- <el-button :disabled="!leafAreaFlag || getAuth('customer:zhiyv:allot')">分配</el-button> -->
            <!-- <el-button :disabled="!leafAreaFlag || getAuth('customer:zhiyv:packet')">生成数据包</el-button> -->
            <el-button :disabled="!leafAreaFlag || getAuth('customer:zhiyv:equipment')">批量关联设备卡</el-button>
            <el-button :disabled="getAuth('customer:zhiyv:analyse')" @click="showAnalysisOptionsDialog">数据分析</el-button>
            <el-button @click="showExportDialog" :disabled="getAuth('customer:zhiyv:export')">导出</el-button>
          </template>
          <el-popover :width="320" trigger="click" @show="showSaturationStatistics">
            <template #reference>
              <el-button v-if="activeIndex == '4'" :disabled="getAuth('customer:open:saturability')">饱和度计算</el-button>
              <el-button v-else :disabled="getAuth('customer:zhiyv:saturability')">饱和度计算</el-button>
            </template>
      ...
      >
        ...
        <el-table-column fixed="left" label="操作" width="110" class-name="table-operation">
          <template #default="scope">
            ...
            <el-button v-if="activeIndex == '4'" link type="primary" @click="edit(scope.row)" :disabled="!leafAreaFlag || getAuth('customer:open:edit') || scope.row.deathFlag == '0'">
              编辑
            </el-button>
            <el-button v-else link type="primary" @click="edit(scope.row)" :disabled="!leafAreaFlag || getAuth('customer:zhiyv:edit') || scope.row.deathFlag == '0'"> 编辑 </el-button>
            <!-- <el-button
                link
        ...
        <el-table-column show-overflow-tooltip prop="haveMarketOrder" label="是否有市场化订单" width="150" />
        <el-table-column v-if="activeIndex == '3'" prop="haveMealOrder" label="	是否有助餐订单" width="150" />
  ...
  <CustomerExportDialog ref="exportRef" :multipleSelection="multipleSelection" :queryForm="queryForm" />
  <AnalysisOptionDialog ref="analysisOptionRef" />
</template>
<script setup lang="ts">
// import { ElButton, ElLink, ElCard, ElInput, ElSpace } from 'element-plus'
...
async function getTableList(filterForm) {
  if (filterForm) {
    ...
    filterKeyCount.value = Object.keys(cleanObj).length - (activeIndex.value == '3' ? 4 : 5)
  }
  loading.value = true
...
onMounted(() => {
  if (route.fullPath == '/plat') {
    activeIndex.value = '4' // 开放平台
    queryForm.isMarkerCustomer = 1
    queryForm.type = 4
  } else {
    activeIndex.value = '3' // 智宇
    queryForm.isMarkerCustomer = undefined
  }
  getTableList(null)
})
</script>
