<template>
    ...
    <el-card shadow="never" class="card">
      ...
      <div class="query-form">
        <el-space wrap :size="12">
          <el-button type="primary" @click="addCustomer" :disabled="!leafAreaFlag || getAuth('customer:index:add')"> 添加客户 </el-button>
          <!-- <el-button :disabled="!leafAreaFlag || getAuth('customer:index:allot')">分配</el-button> -->
          <!-- <el-button :disabled="!leafAreaFlag || getAuth('customer:index:packet')">生成数据包</el-button> -->
          <el-button :disabled="!leafAreaFlag || getAuth('customer:index:equipment')">批量关联设备卡</el-button>
          <el-button :disabled="getAuth('customer:index:analyse')" @click="showAnalysisOptionsDialog">数据分析</el-button>
          <el-button @click="showExportDialog" :disabled="getAuth('customer:index:export')">导出</el-button>
          <el-popover :width="320" trigger="click" @show="showSaturationStatistics">
            <template #reference>
              <el-button :disabled="getAuth('customer:index:saturability')">饱和度计算</el-button>
            </template>
            <div v-loading="saturationStatisticsLoading">
              <el-space spacer="|">
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.generalInfoPct || 0 : saturationStatistics.generalInfoNonePct || 0 }}% </strong>
                  <span>总体</span>
                </div>
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.baseInfoPct || 0 : saturationStatistics.baseInfoNonePct || 0 }}% </strong>
                  <span>基础</span>
                </div>
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.extenInfoPct || 0 : saturationStatistics.extenInfoNonePct || 0 }}% </strong>
                  <span>延伸</span>
                </div>
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.demandInfoPct || 0 : saturationStatistics.demandInfoNonePct || 0 }}% </strong>
                  <span>需求</span>
                </div>
              </el-space>
              <el-divider style="margin: 5px 0; border-color: #f5f3f0" />
              <el-space spacer="|">
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.visitInfoPct || 0 : saturationStatistics.visitInfoNonePct || 0 }}% </strong>
                  <span>拜访</span>
                </div>
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.familyInfoPct || 0 : saturationStatistics.familyInfoNonePct || 0 }}% </strong>
                  <span>家庭</span>
                </div>
                <div class="saturation-box">
                  <strong> {{ xiamenFlag ? saturationStatistics.healthInfoPct || 0 : saturationStatistics.healthInfoNonePct || 0 }}% </strong>
                  <span>健康</span>
                </div>
              </el-space>
            </div>
          </el-popover>
          <el-input v-model="queryForm.accurateSearch" placeholder="请输入完整的证件号码/联系电话/ID" style="width: 300px" @change="accurateSearch" v-show="isAccurate">
            <template #suffix>
      ...
      >
        ...
        <el-table-column fixed="left" label="操作" width="110" class-name="table-operation">
          <template #default="scope">
            <el-button link type="primary" @click="detail(scope.row)" :disabled="getAuth('customer:index:detail')"> 查看 </el-button>
            <el-button link type="primary" @click="edit(scope.row)" :disabled="!leafAreaFlag || getAuth('customer:index:edit') || scope.row.deathFlag == '0'"> 编辑 </el-button>
            <!-- <el-button
                link
                type="primary"
        ...
        <el-table-column fixed="left" prop="id" label="客户ID" width="160" />
  ...
  <CustomerExportDialog ref="exportRef" :multipleSelection="multipleSelection" :queryForm="queryForm" :total="total" />
  <AnalysisOptionDialog ref="analysisOptionRef" :params="queryForm" />
</template>
<script setup lang="ts">
import { useCache } from '@/hooks/web/useCache'
import { ref, reactive, onMounted } from 'vue'
import { Filter, Search } from '@element-plus/icons-vue'
import { getCustomerPage, getSaturationStatistics, getCustomerMsg } from '@/api/customer/index'
...
function addCustomer() {
  if (!leafAreaFlag.value || getAuth('customer:index:add')) return
  formRef.value.show()
}
...
onMounted(() => {
  getTableList(null)
  if (sessionStorage.getItem('add-customer') === 'activity') {
    // 从助老管理-活动管理-新增活动跳转过来的
    addCustomer()
    sessionStorage.removeItem('add-customer')
  }
})
</script>
<style scoped>
